...
export class LoginService {
    ...
    static readLoginInfo(packet: PacketReader) {
        ...
        const temp_ban = packet.readLong();
        ...
        return {id, hashedPassword, gender, banned, temp_ban, pin, pic, characterSlots, tos, language};
    }

    private static async loginOk(preLoginClient: PreLoginClient, loginInfo: any): Promise<number> {

        if (loginInfo.banned) return 3;
        // temp_ban is a Windows FILETIME (100ns ticks since 1601-01-01). Convert to Date.
        const ftMs = Number(loginInfo.temp_ban) / 10000 - 11644473600000;
        if (ftMs > Date.now()) return 3;
        ...
        // Client has no CONFIRM_EULA flow and skips the TOS screen, so treat ToS as accepted here.

        const alreadyLoggedIn = [...LoginServer.instance.loginStore.values()].some(c => c.name === preLoginClient.username);
        if (alreadyLoggedIn) {
            LoginServer.instance.logger.warn(`User ${preLoginClient.username} tried to login when already logged in`);
            return 7;
        }
