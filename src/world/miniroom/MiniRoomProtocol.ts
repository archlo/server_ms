...
export function miniRoomProtocolByValue(value: number): MiniRoomProtocol | null {
  for (const k of Object.keys(MiniRoomProtocol)) {
    const v = (MiniRoomProtocol as any)[k];
    if (typeof v === 'number' && v === value) return v as MiniRoomProtocol;
  }
  return null;
}
