...
const opToHeaderMap = new Map<number, InHeader>();
for (const [k, v] of Object.entries(InHeader)) {
  if (typeof k === 'string' && typeof v === 'number') {
    opToHeaderMap.set(v, v as unknown as InHeader);
  }
}

export function inHeaderByOp(op: number): InHeader | null {
  return opToHeaderMap.get(op) ?? null;
}

const SPAM_HEADERS = new Set([
  InHeader.SECURITY_PACKET,
  InHeader.MOB_MOVE,
  InHeader.USER_MOVE,
  InHeader.PASSIVESKILL_INFO_UPDATE,
  InHeader.USER_CHANGE_STAT_REQUEST,
  InHeader.SUMMONED_MOVE,
  InHeader.USER_TEMPORARY_STAT_UPDATE_REQUEST,
  InHeader.USER_CALC_DAMAGE_STAT_SET_REQUEST,
  InHeader.MOB_APPLY_CTRL,
  InHeader.USER_QUEST_REQUEST,
  InHeader.CHECK_SPW_REQUEST,
  InHeader.NPC_MOVE,
  InHeader.PET_MOVE,
  InHeader.DRAGON_MOVE,
  InHeader.USER_HIT,
  InHeader.USER_EMOTION,
]);

export function isSpamHeader(op: InHeader): boolean {
  return SPAM_HEADERS.has(op);
}
