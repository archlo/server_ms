...
const outHeaderByOpMap = new Map<number, OutHeader>();
for (const [k, v] of Object.entries(OutHeader)) {
  if (typeof k === 'string' && typeof v === 'number') {
    outHeaderByOpMap.set(v, v as unknown as OutHeader);
  }
}

export function outHeaderByOp(op: number): OutHeader | null {
  return outHeaderByOpMap.get(op) ?? null;
}
